...
function seedInitialData() {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    db.run(
      "INSERT OR IGNORE INTO dim_rayon (rayon_id, code, name) VALUES (?, ?, ?)",
  ...
  for (const m of mustHaves) {
    db.run(
      "INSERT OR IGNORE INTO must_have_program_config (line_code, line_name, target_penetration_pct, is_active) VALUES (?, ?, ?, 1)",
      [m.code, m.name, m.pct]
    );
  }

  // 6. Configurable NPL Campaigns
  console.log('Seeding NPL campaigns...');
  db.run(
    "INSERT OR IGNORE INTO npl_campaign (campaign_id, campaign_name, start_date, target_m1_penetration_pct, target_m3_penetration_pct, is_active) VALUES (?, ?, '2026-01-01', 70.0, 85.0, 1)",
    ['DELI_NPL', 'Deli Wafer Roll NPL Campaign']
  );
  db.run(
    "INSERT OR IGNORE INTO npl_campaign (campaign_id, campaign_name, start_date, target_m1_penetration_pct, target_m3_penetration_pct, is_active) VALUES (?, ?, '2026-03-01', 70.0, 85.0, 1)",
  ...
  for (const r of rules) {
    db.run(
      "INSERT OR IGNORE INTO incentive_rule (rule_id, component_number, component_name, mangkok_amount, min_achievement_pct, max_achievement_pct, payout_rule_type, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)",
  ...
  for (const set of settings) {
    db.run(
      "INSERT OR IGNORE INTO business_settings (key, value, description) VALUES (?, ?, ?)",
      [set.key, set.val, set.desc]
    );
  }

  // 10. Initial App Users
  ...
  for (const u of users) {
    db.run(
      "INSERT OR IGNORE INTO app_user (user_id, username, password_hash, full_name, role, scope_spv_id, scope_salesman_id, can_upload_sales, can_edit_customer, can_edit_target, can_manage_incentive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [u.id, u.username, u.pass, u.name, u.role, u.spv, u.sales, u.upload, u.edit_c, u.edit_t, u.inc]
    );
  }

  // 11. Store Loyalty Programs (media_1789304819065.png)
  console.log('Seeding Store Loyalty Programs...');
  db.run(
    `INSERT OR IGNORE INTO store_loyalty_program (program_id, program_name, program_type, product_focus, start_date, end_date, is_active)
